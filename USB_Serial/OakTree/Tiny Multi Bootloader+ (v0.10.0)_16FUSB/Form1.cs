...
namespace TinyPicBootloader
{
    public partial class Form1 : Form
    {
        ...
        bool isConfig;
//$$$
        bool isConnected16FUSB;
        bool isSelected16FUSB;
//%%%
...
//$$$
            public OaktreeLab.USBDevice.HIDSimple device;
//%%%
...
//$$$
                ...
                {
                    int brate = Convert.ToInt32(serialPort.BaudRate);
                    int SPBRG = 6000000/brate - 1;    //SPBRG = (24MHz/BaudRate)/4 -1
                    byte SPBRGL = (byte)(SPBRG%256);
                    byte SPBRGH = (byte)(SPBRG/256+128);
                    device.Send( SPBRGL, SPBRGH );
                    device.Send( 0x00, 0x02 );
                }
//%%%
                if (verbose)
...
//$$$
                        ...
                        {
                            device.Send( memBlock[0] );
                            device.Send( memBlock[1] );
                        }
//%%%
...
//$$$
                ...
                {
                    while ((answer = device.Receive1()) != 0x4B) // 0x4B = 'K', erase completed ; 0x45 = 'E', page erased
                    {
                        textRawTransfert.AppendText(answer.ToString("X2") + ", ");
...
//$$$
//              if ((checkVirtualPIC.Checked) || (serialPort.IsOpen))
                if ((checkVirtualPIC.Checked) || (serialPort.IsOpen) || (isSelected16FUSB))
//%%%
                {
                    //--- rem stop
...
//$$$
                        ...
                        {
                            device.Send( 0x00, 0x08 );
                            device.Send( serBuffer[0] );
                        }
//%%%
                    }
                    textRawTransfert.AppendText("\r\nsend: C1");
...
//          if ((serialPort.IsOpen == true)||(checkVirtualPIC.Checked))
            if ((serialPort.IsOpen) || (checkVirtualPIC.Checked) || (isSelected16FUSB))
//%%%
            {
...
//$$$
                    ...
                    {
                        device.Send( 0x00, 0x10 );
                        System.Threading.Thread.Sleep(int.Parse(textRTSTrue.Text)); // wait some ms
                        device.Send( 0x00, 0x20 );
                    }
//%%%
...
//$$$
	    if (isConnected16FUSB)
                listDetectedCOM.Items.Add("16FUSB");
//%%%
        }
...
//$$$
            if (isConnected16FUSB & (selectedCOM.Text == "16FUSB"))
            {
                isSelected16FUSB = true;
                if (radioResetDTR.Checked)
                {
                    radioResetDTR.Checked = false;
                    radioResetRTS.Checked = true;
                }
                if (checkPowerRTS.Checked)
                    checkPowerRTS.Checked = false;
                radioResetDTR.Enabled = false;
                checkPowerRTS.Enabled = false;
            }
            else
            {
                isSelected16FUSB = false;
                radioResetDTR.Enabled = true;
            }
//%%%
        }
...
//$$$
            ...
            if ( device.Open( 0x04d8, 0x0628 ) )
                isConnected16FUSB = true;
            else
                isConnected16FUSB = false;
//%%%
...
//          if (serialPort.IsOpen)
            if ((serialPort.IsOpen) || (isSelected16FUSB))
//%%%
            {
