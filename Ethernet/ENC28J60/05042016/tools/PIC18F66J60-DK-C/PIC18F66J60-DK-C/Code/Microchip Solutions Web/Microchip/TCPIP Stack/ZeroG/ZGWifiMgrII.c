...
WiFiAuthRequest( void * const ptrRequest, tZGVoidInput *appOpaquePtr)
{
   ...
   if( (APPCXT.capInfo[0] & kWifiMgrCapBitPrivacy) == 0 ||
       (APPCXT.securityInfo[0] != 0 || APPCXT.securityInfo[1] != 0))
   {
                 ...
                 tZGU16 len, tZGVoidInput *appOpaquePtr)
{
    ...
    else
    {
         ...
         else if(result == kZGResultInvalidParams)
         {
...
#if defined ( ZG_CONFIG_CONSOLE )
         else if(result == kZGResultAuthRefused)
         {
                  ...
                  tZGU16 len, tZGVoidInput *appOpaquePtr)
{
   ...
   else
   {
     ...
     if(result == kZGResultInvalidParams)
     {
