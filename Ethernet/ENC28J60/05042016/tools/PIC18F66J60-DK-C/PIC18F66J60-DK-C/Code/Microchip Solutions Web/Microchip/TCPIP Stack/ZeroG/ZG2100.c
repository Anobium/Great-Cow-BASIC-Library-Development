...
//============================================================================
#define kSnap       (0xaa)
#define kSnapCtrl   (0x03)
#define kSnapType   (0x00)
...
//                                  RAW Constants
//============================================================================
#define kENCRdPtrId (0)
#define kENCWrPtrId (1)

#define kZGRxBufferRAWId    (kZGRawId0)
#define kZGTxBufferRAWId    (kZGRawId1)
#define kZGInvalidRAWId     (0xff)

#define kZGRAWUnmounted             (0)
#define kZGRAWScratchMounted        (1)
#define kZGRAWDataBufMounted        (2)
#define kZGRAWMgmtBufMounted        (3)
...
#define MAX_CREATE_TXBUF_RETRY  10000
...
static void SyncENCPtrRAWState(tZGU8 encPtrId)
{
    ...
    else
    {
        ...
        {
            if ( encPtrRAWId[1 - encPtrId] == kZGRxBufferRAWId )
            {
...
{
     /* See comment in MACProcess */
     return ( RawMgmtRxInProgress || RawMgmtAppWaiting );
}
...
static Boolean CreateTxBuffer(tZGU16 srcDest)
{
    ...
    // get number of bytes available in RAW tx buffer
    bufAvail = Read16BitZGRegister(kZGCOMRegWrtFifo0ByteCnt);

    if ( bufAvail >= (4ul+MAX_PACKET_SIZE+4ul) )
    {
        // create the new tx buffer
        byteCount = ZGRawMove(kZGTxBufferRAWId, srcDest, true, (4ul+MAX_PACKET_SIZE+4ul));
        if ( byteCount == 0 )
        {
#if 0
