...
#define kComStIdle              (1)
#define kComStIntService        (2)
#define kComStReadOperation     (3)
#define kComStWriteOperation    (4)
#define kComStWriteWaitFifoDone (5)
#define kComStReadWaitFifoDone  (6)
...
static tZGVoidReturn ChipReset(tZGVoidInput)
{
    ...
    } while((value & (tZGU16)kZGCOMRegNotInResetMask) == 0);
    ...
    do
    {
        if(COMCXT.state == kComStIdle)
        {
            ...
            else if(COMCXT.bRdWtReady == kZGBoolTrue)
            {
                ...
                {
                    /* send managment message (which was previously written to Raw Tx buffer) */
                    if ( ZGSendRAWManagementFrame(RWCXT.len) == kZGBoolFalse )
                    {
