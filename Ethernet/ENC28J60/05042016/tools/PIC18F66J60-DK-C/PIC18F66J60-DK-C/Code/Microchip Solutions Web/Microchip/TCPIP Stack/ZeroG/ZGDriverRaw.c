...
Boolean ZGRawSetIndex(tZGU16 rawId, tZGU16 index)
{
    ...
    while (1)
    {
        ...
        if ((regValue & kZGHostRAWStatusBusyMask) == 0)
        {
            return kZGBoolTrue;
        }
...
Boolean ZGRawGetByte(tZGU16 rawId, tZGU8 *pBuffer, tZGU16 length)
{
    ...
    if ( (rawId==0) && gRxIndexSetBeyondBuffer && (zgBufferRAWState[0] == 2) ) // 2==kZGRAWBufferMounted
    {
        char buf[10];
...
Boolean ZGRawSetByte(tZGU16 rawId, tZGU8 *pBuffer, tZGU16 length)
{
    ...
    if ( (rawId==0) && gRxIndexSetBeyondBuffer && (zgBufferRAWState[0]==2) )
    {
        char buf[10];
...
tZGU16 ZGMACIFService(tZGVoidInput)
{
   ...
   //     putrsUART("z\r\n");
    ...
    if(ZGRawGetByte(kRxPipeRAW, (tZGU8*)&(gPreBuf.type), 2) == 0)
    {
        /* failure */
      //  putrsUART("g\r\n");
        byteCount = 0;
