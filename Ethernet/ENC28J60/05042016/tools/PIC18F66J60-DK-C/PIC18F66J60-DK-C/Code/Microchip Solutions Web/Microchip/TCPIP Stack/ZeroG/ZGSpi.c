...
//============================================================================
static tZGVoidReturn zgSpiISRInternal(tZGVoidInput);
...
//  pSpiTxPreambleBuffer =  SPICXT.pTxPreambleBuffer;
//  spiTxPreambleLength  =  SPICXT.txPreambleLength;
//  pSpiTxBuffer         =  SPICXT.pTxBuffer;
//    pSpiTxRomBuffer      =  SPICXT.pTxRomBuffer;
    spiTxLength          =  SPICXT.txPreambleLength + SPICXT.txLength;
//  pSpiRxPreambleBuffer =  SPICXT.pRxPreambleBuffer;
//  spiRxPreambleLength  =  SPICXT.rxPreambleLength;
//  pSpiRxBuffer         =  SPICXT.pRxBuffer;
//  spiRxBufferLength    =  SPICXT.rxBufferLength;
    spiRxLength          =  SPICXT.rxPreambleLength + SPICXT.rxBufferLength + SPICXT.excessRxLength;
    spiCurrentLength     =  0;
    ...
    // Tx/Rx each byte by calling zgSpiISRInternal (no SPI interrupt in this demo)
    while ( ((spiCurrentLength + 1) <= spiTxLength) || ((spiCurrentLength + 1) <= spiRxLength) )
    {
        zgSpiISRInternal();
    }

    // now that all bytes transmitted, call ISR one more time so proper cleanup is done
    zgSpiISRInternal();
...
/*****************************************************************************
 * FUNCTION: zgSpiISRInternal
 *
 * RETURNS:  None
 *
 * PARAMS:   None
 *
 *  NOTES: Normally this would be an actual interrupt routine, but, since the Microchip
 *         application uses polled mode, this function is directly called for each
 *         byte of Tx/Rx.
 *****************************************************************************/
static tZGVoidReturn zgSpiISRInternal(tZGVoidInput)
{
    ...
    /* clear the interrupt by reading the byte */
    /* read the rxByte */
    if(spiCurrentLength)
    {
        rxByte  = ZG_SSPBUF;
    ...
    /* do we have any rx work left */
    if ( (spiCurrentLength > 0) && (spiCurrentLength <= spiRxLength) )
    {
        if ( (spiCurrentLength-1) < SPICXT.rxPreambleLength )
        {
            *(SPICXT.pRxPreambleBuffer+(spiCurrentLength-1)) = rxByte;
        }
        else if ( (spiCurrentLength-1) < (SPICXT.rxPreambleLength + SPICXT.rxBufferLength) )
        {
            *(SPICXT.pRxBuffer+(spiCurrentLength-1-SPICXT.rxPreambleLength)) = rxByte;
        }
    }

    /* do we have any tx work left */
    if ( spiCurrentLength < spiTxLength )
    {
        if ( spiCurrentLength < SPICXT.txPreambleLength )
        {
            txByte = *(SPICXT.pTxPreambleBuffer + spiCurrentLength);                /* get the next tx preamble buffer byte */
        }
        else
        {
            // if a RAM location that we are reading the next tx byte from
            if (SPICXT.pTxBuffer != 0)
            {
                txByte = *(SPICXT.pTxBuffer + spiCurrentLength - SPICXT.txPreambleLength);  /* get the next tx buffer byte */
            }
            // else must a ROM location we are reading the next tx byte from
            else if (SPICXT.pTxRomBuffer != 0)
            {
                txByte = *(SPICXT.pTxRomBuffer + spiCurrentLength - SPICXT.txPreambleLength);   /* get the next tx buffer byte */
            }
            else
            {
    ...
    {
        txByte = 0xff;          /* use a dummy byte once we are through with the tx buffer */
    }

    spiCurrentLength++;         /* point to the next byte in each buffer */

    /* do we have any work left */
    if ( (spiCurrentLength <= spiTxLength) || (spiCurrentLength <= spiRxLength) )
    {
        /* write the txByte */
        ZG_SSPBUF = txByte;
        WaitForDataByte();
