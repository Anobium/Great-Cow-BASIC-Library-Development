...
int listen( SOCKET s, int backlog )
{
    ...
    if ((ps->bsdState >= SKT_BOUND) && (ps->SocketType == SOCK_STREAM))
    {
      // put this socket in listen mode
       ...
       while(ps->backlog--)
       {
            if((clientSockID = TCPOpen(0, TCP_OPEN_SERVER, ps->localPort, TCP_PURPOSE_BERKELEY_SERVER)) == INVALID_SOCKET)
            {
                return SOCKET_ERROR;
            }
            for(socketcount =0; socketcount < BSD_SOCKET_COUNT; socketcount++)
            {
                if(BSDSocketArray[socketcount].SocketID == INVALID_SOCKET)
                {
                    BSDSocketArray[socketcount].SocketID = clientSockID;
                    BSDSocketArray[socketcount].backlog  = 0;
                    ...
                    BSDSocketArray[socketcount].SocketType = ps->SocketType;
					break;
                }
            }
       }
