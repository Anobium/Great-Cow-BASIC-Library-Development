...
'   EtherBuffer(1) = 0xFF : EtherBuffer(2) = 0xFF
'   EtherBuffer(3) = 0xFF : EtherBuffer(4) = 0xFF
'   EtherBuffer(5) = 0xFF : EtherBuffer(6) = 0xFF
'   EtherBuffer(7) = 0x08 : EtherBuffer(8) = 0x06
'   EtherBuffer(9) = ipaddr(1) : EtherBuffer(10) = ipaddr(2)
'   EtherBuffer(11) = ipaddr(3) : EtherBuffer(12) = ipaddr(4)
'   Calcchecksum(1 , 11 , 13)                           'Checksum will be in EtherBuffer(13)(14)
'
'HSerPrintByteCRLF EtherBuffer(13)
'HSerPrintByteCRLF EtherBuffer(14)
'HSerPrintCRLF 2
'
'    ENC28J60_writeReg(epmcsl , EtherBuffer(14))    'Checksum for ARP packets
'    ENC28J60_writeReg(epmcsh , EtherBuffer(13))    'Checksum for ARP packets
'
'   ENC28J60_writeReg(epmm0 , 0x3F)           'Pattern Match Mask0 - Broadcast (FF FF FF FF FF FF)
'   ENC28J60_writeReg(epmm1 , 0x30)           'Pattern Match Mask1 - Packet type (ARP - 08 06)
'   ENC28J60_writeReg(epmm4 , 0xC0)           'Pattern Match Mask4 - Target IP first 3 byte (ARP packet for us)
'   ENC28J60_writeReg(epmm5 , 0x03)           'Pattern Match Mask5 - Target IP last 1 byte
'
''   Value.erxfcon_pmen = 1                                   'Pattern Match enable (ARP only)
''   Value.erxfcon_ucen = 1                                   'Unicast enable
'   ENC28J60_bitSet( erxfcon , erxfcon_ucen + erxfcon_pmen )
''   Value = 0
''   Value.erxfcon_bcen = 1                                   'Broadcast disable
'
'    ENC28J60_bitClear( erxfcon , erxfcon_bcen )
...
'    ANSI ( 0, 15 )
    ...
    If ENC28J60_temp = 0 Then
        HSerPrint "No packet received"
        HSerPrintCRLF
        Exit Sub
    End If
    ...
    Length = etherbuffer(3)
    Length_h = etherbuffer(4)
...
Sub Calcchecksum(startptr As Word , Endptr As Word , Result As Word)
  ...
  For Ii = Startptr To Endptr Step 2
      HSerPrint etherbuffer(ii+1)
      HSerPrintCRLF
      tmp = etherbuffer(ii + 1)
      Sum = [long]Sum + [long]tmp
      HSerPrint ii
      HSerPrint ")sum = "
      HSerPrint hex(sum_e)
      HSerPrint hex(sum_u)
