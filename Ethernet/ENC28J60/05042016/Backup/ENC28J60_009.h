...
sub eth_read( optional in etherGetLength as word = 1 )
  ...
  for eth_counter = 0 to buf_len -1
    SSPBUF = 0
    Wait While BF = Off
    etheretherbuffer( eth_counter ) = SSPBUF
  next
...
Sub Enc28j60packetreceive( in maxlen As Word )
    Dim eth_counter As Word
    Dim ENC28J60_temp Alias eth_counter as word

    HserPrint "Sub Enc28j60packetreceive"
    HSerPrintCRLF

    ENC28J60_temp = ENC28J60_ReadReg (eir)
    HserPrint ByteToBin ( [byte]ENC28J60_temp )
    ...
    ENC28J60_temp   = ENC28J60_ReadReg(ERDPTL);
    ENC28J60_temp_h = ENC28J60_ReadReg(ERDPTH);

    'Show the read pointer to the start of the received packet

    HSerPrint "Present nextpacketpntr "
    HSerPrint Hex(ENC28J60_temp_h)
    HSerPrint Hex(ENC28J60_temp)
    HSerPrintCRLF

    'read the next packet pointer
    Enc28j60readbuffer(1)
    Nextpacketptr = Buffer(1)
    Enc28j60readbuffer(1)
    Nextpacketptr_h = Buffer(1)
    HSerPrint "Nextpktpntr " : HserPrint Hex( Nextpacketptr_h ) : HserPrint Hex( Nextpacketptr ) : HSerPrintCRLF
    ...
    Enc28j60readbuffer(1)
    RxStat_h = Buffer(1)
    HSerPrint "Packet Length " : HserPrint Hex( RsStat_h ) : HserPrint Hex(RsStat ) : HSerPrintCRLF
    ...
    ENC28J60_writeReg(ERXRDPTH, [byte]nextpacketptr_h )
